...
function fileComplaint(data) {
  try {
    ...
    complaintsheet.appendRow([data.BusinessName, data.ReviewType, data.ComplainantName, data.Email, data.Mobile, data.ComplainDesc, attachmentURL, "Pending"]);
...
function approveBusinessRegistration(bID) {
  try {
    var dataRange = mastersheet.getRange(1,1,mastersheet.getLastRow(),2).getValues();
    Logger.log(dataRange);
    var rowIndex = 0;
    for(var i = 0; i < dataRange.length; i++) {
      if(dataRange[i][0].toString() == bID) {
...
function verifyReview(bID) {
  try {
    var dataRange = complaintsheet.getRange(1,1,complaintsheet.getLastRow(),1).getValues();
    Logger.log(dataRange);
    var rowIndex = 0;
    for(var i = 0; i < dataRange.length; i++) {
      if(dataRange[i][0].toString() == bID) {
...
function sendEmailReviewNotif(bID) {
  try {
    ...
    var body = "A review with your registered business: " + businessName + " has been sent and is now pending for review";
...
function sendEmailReviewResult(bID) {
  try {
    ...
    var subject = "Review Notif";
    var body = "A review with your registered business: " + businessName + " has been validated and is now reflected on your badge score";
    GmailApp.sendEmail(recipient,subject,body);
  } catch (e) {
    Logger.log(e)
  }
...
function sendEmailOTP(recipient) {
  try {
    ...
    GmailApp.sendEmail(recipient,subject,body,{cc: "julius.petre@gmail.com"});
  } catch (e) {
    Logger.log(e);
  }
}
...
function sendRegistrationBadge(bID) {
  try {
    ...
    var body = "Your online business registration is approved. Attached is a basic entry level badge that you can use as you see fit";
...
function getAuthToken(mobileNum){
  ...
  try {
    let htmlTblStat;
      ...
      var tableRequestURL = "https://api-uat.unionbankph.com/partners/sb/partners/v1/oauth2/token";
...
function sendOTP(authToken,mNum){
  let htmlTblStat;
      ...
      var tableRequestURL = "https://api-uat.unionbankph.com/partners/sb/otp/v1/request";
      ...
      'x-ibm-client-secret':'eU2kS4qI8aY3fL1iF5cL0dY6oV2rK3sM8aT7kM8oO0lT7mG8eS',
...
function validatePIN(requestID,pin){
  let htmlTblStat;
      ...
      var tableRequestURL = "https://api-uat.unionbankph.com/partners/sb/otp/v1/verify";
